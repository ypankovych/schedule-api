...
class Group(Resource):
    args = {
        ...
        'chair': fields.Bool(missing=True)
    }

    @staticmethod
    @use_kwargs(args)
    @swag_from('../docs/group.yaml')
    def get(group, from_date, to_date, chair):
        result = parse({'group': group.encode(default_encoding)}, from_date, to_date, chair)
        if result:
            return result, 200
...
class Teacher(Resource):
    ...
    def get(teacher, from_date, to_date, chair):
        result = parse({'teacher': teacher.encode(default_encoding)}, from_date, to_date, chair)
        if result:
            return result, 200
        return {'error': 'Schedule not found'}, 404
